//const chunkSize = 60
OPC.slider("chunkSize", 60, 10, 300, 5)
...
//const maxAngle = 10
OPC.slider("maxAngle", 10, 1, 90, 1)
//const triggerFade = 1000 //ms
OPC.slider("triggerFade", 1000, 100, 10000, 100)
//const useCorners = true
OPC.toggle("useCorners", true)
//const useLogColoring = true
OPC.toggle("useLogColoring", true)
let coloring = useLogColoring ? count => log(count) + 1 : count => count
//const frustumCulling = true
OPC.toggle("frustumCulling", true)
//const viewAngle = 90
OPC.slider("viewAngle", 90, 10, 180, 5)
//const furstumAngleOffset = 90
OPC.slider("viewDirection", 90, 0, 360, 5)

OPC.color("defaultColorName", "#ffffff")
OPC.color("sortColorName", "#009600")
OPC.color("triggerColorName", "#8F31C6F")

function parameterChanged(name, value) {
  if (name === "useLogColoring") {
    coloring = useLogColoring ? count => log(count) + 1 : count => count
  }
  if (name === "defaultColorName") {
    defaultColor = color(defaultColorName)
  }
  if (name === "sortColorName") {
    sortColorName = color(sortColorName)
  }
  if (name === "triggerColorName") {
    triggerColor = color(triggerColorName)
  }
}
...
class Chunk {
  ...
  testSort(playerX, playerY) {
    ...
    const culled =
      frustumCulling &&
      min(
        wrappedAngleDist(viewDirection, angleNow),
        wrappedAngleDist(viewDirection, angleNow01),
        wrappedAngleDist(viewDirection, angleNow10),
        wrappedAngleDist(viewDirection, angleNow11)
      ) >
        viewAngle / 2 &&
      !(
        playerX > this.x &&
